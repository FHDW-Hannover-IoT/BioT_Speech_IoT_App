 ...
 *
 * <p>buildSegmented: splits a flat entry list into multiple LineDataSets at
 * "gap" positions (where the inter-sample interval exceeds 3× the median).
 * Each segment becomes its own dataset so MPAndroidChart does not draw a
 * bridging line across periods with no data (e.g. offline windows, paused
 * sensors).
 */
public final class GraphUtils {
     ...
     */
    public static List<LineDataSet> buildSegmented(ArrayList<Entry> entries, String label, int color) {
        if (entries == null || entries.isEmpty()) return Collections.emptyList();
        if (entries.size() == 1) return Collections.singletonList(makeDataSet(entries, label, color));
