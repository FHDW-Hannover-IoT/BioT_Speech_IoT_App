...
/**
 * VoiceInputManager ─────────────────────────────────────────────────────────────────────────────
 * Wraps Android's {@link SpeechRecognizer} for push-to-talk voice input WITHOUT showing the Google
 * dialog popup.
 *
 * <p>Why not use startActivityForResult? ───────────────────────────────────── The approach in
 * MainActivity used startActivityForResult which always shows Google's speech dialog. This class
 * uses SpeechRecognizer directly so: - No popup — the UI button itself is the visual cue - The
 * caller controls the listening state (startListening / stopListening) - Results are delivered via
 * {@link VoiceResultListener} callback
 *
 * <p>Lifecycle rules (IMPORTANT) ───────────────────────────── • Must be created on the MAIN thread
 * (SpeechRecognizer requirement). • Call {@link #destroy()} in Activity.onDestroy() to avoid leaks.
 * • Do NOT call startListening() while already listening — call stopListening() first, or check
 * {@link #isListening()}.
 *
 * <p>Usage in MainActivity ──────────────────────
 *
 * <pre>
...
public class VoiceInputManager {
    ...
    public interface VoiceResultListener {
        ...
        /**
         * Called when the recognizer has started listening (microphone is open). Use this to update
         * the button icon to "recording" state.
         */
        default void onListeningStarted() {}

        /**
         * Called when the recognizer finishes (either result or error). Use this to restore the
         * button icon to "idle" state.
         */
    ...
    /**
     * @param context Must be an Activity context (NOT application context). SpeechRecognizer
     *     requires it.
    ...
    /**
     * Start listening. If already listening, this call is ignored. Requires RECORD_AUDIO permission
    ...
    /**
     * Stop listening early (e.g. user releases a push-to-talk button). The recognizer will process
    ...
    /**
     * Release all resources. Must be called in Activity.onDestroy(). After this, the instance
     * cannot be reused.
     */
    public void destroy() {
        recognizer.cancel();
