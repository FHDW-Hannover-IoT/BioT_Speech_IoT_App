...
/**
 * MyEventAdapter ---------------- This RecyclerView.Adapter renders a list of EreignisData (event
 * entries) into the event_list_item layout.
 *
 * <p>Each event row shows: • Sensor type (ACCEL / GYRO / MAGNET) • Timestamp (converted to readable
 * Date) • Value (the detected threshold-exceeding sensor value) • Axis (X / Y / Z)
 *
 * <p>The adapter itself is intentionally very lightweight: - All logic is handled inside
 * EreignisActivity (sorting/filtering). - The adapter only displays the data given to it.
 */
