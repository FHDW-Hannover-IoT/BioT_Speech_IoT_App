 ...
 * values. - "Sensor/Gyro" : CSV "x,y,z" → interpreted as gyro-like values. - "Sensor/Zeit" : plain
 * ISO-like timestamp string. Data flow: - Timer (ScheduledExecutorService) fires. - Simulator
 * generates random payloads. - Calls {@link MqttHandler#publish(String, String, boolean)} for each
 * topic. - Broker receives PUBLISH and forwards to any subscribers. - Our own client is subscribed,
 * so messages arrive in MqttHandler → MainActivity → TextViews + Room DB.
 */
