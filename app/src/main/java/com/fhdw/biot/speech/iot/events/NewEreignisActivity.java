 ...
 * NewEreignisActivity
 * --------------------
 * Screen where the user can define *event rules* (EditableSensorEvent objects)
 * that determine when a "SensorEreignis" (sensor event) should be raised.
 *
 * Conceptually:
 *   - EreignisActivity  = shows ALREADY TRIGGERED events from DB.
 *   - NewEreignisActivity = lets the user configure WHEN such events
 *                           should be triggered in the future.
 *
 * UI:
 *   - A RecyclerView with rows of type EditableSensorEvent:
 *       • sensorType (Accel / Gyro / Magnet)
 *       • eventType  (e.g. "Sturz", "Schock", "Schwelle überschritten")
 *       • threshold  (float value)
 *   - A "+" button that adds a new editable row.
 *   - Navigation buttons back to:
 *       • MainActivity (home)
 *       • EreignisActivity (event overview)
 *
 * Note:
 *   Currently, only the UI list is implemented. The persistence logic
 *   ("Datenbanklogik hinzufügen") is still a TODO.
 */
public class NewEreignisActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonEreignis.setOnClickListener(view -> {
