 ...
 *
 * <p>Each {@link Rule} pairs a {@link VoiceCommand} with a list of keyword groups. A rule matches
 * when the transcript contains at least one keyword from EVERY required group (AND logic between
 * groups, OR logic within a group).
 *
 * <p>Examples ──────── keywords = [["zeige","öffne"], ["gyro"]] → matches "zeige Gyro" ✓ "öffne den
 * Gyro" ✓ "Gyro" ✗ "zeige" ✗
 *
 * <p>keywords = [["beschleunigung","bewegung","accel"]] → matches any one of those words alone
 *
 * <p>Rule ordering matters: the resolver walks rules top-to-bottom and returns the FIRST match.
 * More specific rules (more keyword groups) MUST come before their shorter / more generic siblings.
 *
 * <p>This dictionary is the source of truth for what the keyword-based fast path recognises.
 * Anything it can't match becomes UNKNOWN, at which point the VoiceCommandExecutor decides whether
 * to fall back to the LLM.
 */
public class VoiceCommandDictionary {
    ...
    private static List<Rule> buildRules() {
        ...
        r.add(
                Rule.of(
                        VoiceCommand.TELL_VALUE,
                        new String[] {"tell", "say", "speak", "what is the value"}));

        // ══ OPERATING MODE
        r.add(
                Rule.of(
                        VoiceCommand.OPMODE_AUTARK,
                        new String[] {"autark", "autonomous", "standalone", "power saving"}));

        r.add(
                Rule.of(
                        VoiceCommand.OPMODE_SUPERVISION,
                        new String[] {"supervision", "monitor", "monitoring"}));

        r.add(Rule.of(VoiceCommand.OPMODE_EVENT, new String[] {"event mode", "trigger mode"}));

        r.add(
                Rule.of(
                        VoiceCommand.OPMODE_IDENTIFICATION,
                        new String[] {"identification", "identify"}));

        r.add(
                Rule.of(
                        VoiceCommand.OPMODE_GET,
        ...
        r.add(
                Rule.of(
                        VoiceCommand.NAV_ACCEL_FILTER_10MIN,
        ...
        r.add(
                Rule.of(
                        VoiceCommand.NAV_GYRO_FILTER_10MIN,
        ...
        r.add(
                Rule.of(
                        VoiceCommand.NAV_MAGNET_FILTER_10MIN,
                        new String[] {"magnet", "hall", "magnetic field"},
                        new String[] {"10", "ten"},
                        new String[] {"minute", "min"}));
        ...
        r.add(
                Rule.of(
                        VoiceCommand.FILTER_LAST_5MIN,
                        new String[] {"5", "five"},
                        new String[] {"minute", "min"}));

        r.add(
                Rule.of(
                        VoiceCommand.FILTER_LAST_10MIN,
                        new String[] {"10", "ten"},
                        new String[] {"minute", "min"}));

        r.add(
                Rule.of(
                        VoiceCommand.FILTER_LAST_30MIN,
                        new String[] {"30", "thirty"},
                        new String[] {"minute", "min"}));

        r.add(
                Rule.of(
                        VoiceCommand.FILTER_LAST_1H,
                        new String[] {"1", "one"},
                        new String[] {"hour"}));

        r.add(
                Rule.of(
                        VoiceCommand.FILTER_LAST_24H,
                        new String[] {"24", "twenty four", "day", "today"},
                        new String[] {"hour", "hours", "day"}));

        r.add(
                Rule.of(
                        VoiceCommand.FILTER_CLEAR,
                        new String[] {"filter", "timeframe", "date", "period", "span", "duration"},
                        new String[] {
                            "reset", "delete", "undo", "cancel", "all", "remove", "revoke",
                            "reverse", "void"
                        }));
        ...
        r.add(
                Rule.of(
                        VoiceCommand.MODE_STREAM,
                        new String[] {"stream", "real time", "live", "continuously"}));

        r.add(
                Rule.of(
                        VoiceCommand.MODE_BURST,
                        new String[] {
                            "burst", "bur", "packet", "packets", "bulk", "package", "packages"
                        }));

        r.add(Rule.of(VoiceCommand.MODE_AVERAGE, new String[] {"average", "median", "mean"}));
        ...
        r.add(
                Rule.of(
                        VoiceCommand.QUERY_ACCEL_VALUE,
                        new String[] {
                            "what", "how", "is", "are", "get", "check", "current", "latest", "value"
                        },
                        new String[] {"acceleration", "accel", "movement", "motion"}));

        r.add(
                Rule.of(
                        VoiceCommand.QUERY_GYRO_VALUE,
                        new String[] {
                            "what", "how", "is", "are", "get", "check", "current", "latest", "value"
                        },
                        new String[] {"gyro", "gyroscope", "rotation", "spin"}));
        ...
        r.add(
                Rule.of(
                        VoiceCommand.QUERY_MAGNET_STATUS,
                        new String[] {
                            "what", "active", "status", "how", "is", "check", "recognized"
                        },
                        new String[] {"magnet", "hall", "magnetic field"}));

        r.add(
                Rule.of(
                        VoiceCommand.QUERY_MIC_LEVEL,
                        new String[] {"what", "how", "loud", "level", "value", "check"},
                        new String[] {"loud", "microphone", "mic", "noise", "sound"}));

        r.add(
                Rule.of(
                        VoiceCommand.QUERY_ANOMALY,
                        new String[] {
                            "anomaly",
                            "anomalies",
                            "strange",
        ...
        r.add(
                Rule.of(
                        VoiceCommand.QUERY_RECENT_EVENTS,
        ...
        r.add(
                Rule.of(
                        VoiceCommand.NAV_MAGNET,
                        new String[] {"magnet", "hall", "magnetic field", "magnetic sensor"}));

        r.add(
                Rule.of(
                        VoiceCommand.NAV_GRAPH,
                        new String[] {
                            "graph", "chart", "diagram", "table", "course", "timeframe", "curve"
                        }));

        r.add(Rule.of(VoiceCommand.NAV_EVENTS, new String[] {"event", "events", "log", "report"}));

        r.add(
                Rule.of(
                        VoiceCommand.NAV_HOME,
                        new String[] {"home", "start", "main", "overview", "dashboard", "back"}));

        r.add(
                Rule.of(
                        VoiceCommand.NAV_SETTINGS,
                        new String[] {"settings", "configuration", "options"}));

        // ══ SYSTEM
        r.add(
                Rule.of(
                        VoiceCommand.SYSTEM_HELP,
